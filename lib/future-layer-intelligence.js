...
function scoreBoostFromTimeline(domain, timeline) {
  ...
  for (const item of safeArray(timeline)) {
    ...
    if (item?.timeline_number === 1) boost += 11;
    else if (item?.timeline_number === 2) boost += 8;
    else if (item?.timeline_number === 3) boost += 6;
    else if (item?.timeline_number === 4) boost += 4;
    else if (item?.timeline_number <= 8) boost += 3;
    else boost += 2;
...
export default function runFutureIntelligenceLayer({
  ...
  const ranked_domains = baseDomains
    .map((d, idx) => {
      ...
      return {
        ...d,
        rank_score: Number((baseScore + boost - idx * 0.01).toFixed(2)),
        normalized_score: Number((Number(d?.normalized_score || baseScore) + boost - idx * 0.01).toFixed(2))
      };
    })
